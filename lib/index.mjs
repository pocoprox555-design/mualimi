...
export async function resolveBook(query = '', { branch = '', subject = '', search: useSearch = false } = {}) {
  const index = await getIndex();
  const wanted = normalizeAr(subject);
  if (wanted) {
    const bySubject = index.books.find((book) => normalizeAr(`${book.subject} ${book.title}`).includes(wanted) && allowedBook(book, { branch }));
    if (bySubject) return bySubject;
  }
  const guessed = guessedBook(index, String(query || ''), '');
  const candidate = guessed ? index.booksById.get(guessed) : null;
  if (candidate && allowedBook(candidate, { branch })) return candidate;
  if (!useSearch) return null;
  const hits = await search(String(query || ''), { branch, limit: 5 });
  ...
  return book && allowedBook(book, { branch }) ? book : null;
}
...
export async function search(query, { bookId = '', subject = '', branch = '', limit = 8, onTrace } = {}) {
  const index = await getIndex();
