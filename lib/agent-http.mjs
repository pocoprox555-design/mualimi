...
function buildSystemPrompt(memory = '', branch = '') {
  return [
    ...
    'إذا كانت نية الطالبة اختباراً: اقرأ الصفحات المطلوبة أولًا ثم استدعِ present_quiz فورًا في نفس الرد — لا تكتب وصفًا أولًا ثم تقول "سأعرض الاختبار" ثم تتوقف. النموذج يجب أن يستدعي الأداة في الاستدعاء نفسه بعد القراءة مباشرة. لا تسأل أسئلة عامة — اسأل فقط عما ورد فعلاً في الصفحات. صوّغ الأسئلة بطريقة الأسئلة الوزارية العراقية — متوسطة الصعوبة، واضحة الصياغة، تشمل تعريفات ومفاهيم وأمثلة وتطبيقات، لا تكون سهلة جدًا ولا صعبة جدًا.',
    ...
    'عندما تستخدم present_quiz: لا تكتب وصفًا طويلًا قبل عرض الاختبار. اقرأ الصفحات واستدعِ الأداة مباشرة. النص قبل الاختبار سطر واحد فقط (مثل "اختبارك جاهز!")، ثم الأداة تعرض الاختبار تلقائيًا. لا تكرر الأسئلة في نص الرد.',
    ...
    'عند الاختبار: اقرأ الصفحات المطلوبة أولًا ثم أنشئ أسئلة تعليمية مفصلة من محتواها الفعلي. كل سؤال يجب أن يكون واضحًا ومفسّرًا وكأن الطالبة لا تعرف المنهج — يبدأ السؤال بتعريف أو تمهيد للمفهوم ثم يطرح السؤال (مثال: إذا كان الدرس عن القواعد النحوية، يعرّف القاعدة أولًا ثم يسأل عنها). أضف شرحًا مفصلاً للإجابة الصحيحة يوضح السبب والطريقة. استخدم present_quiz بأسئلة متنوعة وخيارات وإجابة صحيحة وشرحاً تعليميًا مفصلاً للتصحيح الفوري. الأسلوب ثابت (تعريف + سؤال + شرح) لكن المحتوى يتغير حسب ما ورد في الصفحات الفعلية.',
    '',
...
export function installAgentHttp(app, opts = {}) {
  ...
  const getHeader = (req, name, maxLength) => {
  ...
  const normalizeEndpoint = (value) => {
    ...
    try {
      ...
      if ((parsed.protocol !== 'https:' && !localHttp) || !parsed.host) return ''
    } catch { return '' }
    return endpoint
  }
  const getProviderConfig = (req) => {
    const clientEndpoint = getHeader(req, 'x-ai-endpoint', 500)
    const endpoint = clientEndpoint ? normalizeEndpoint(clientEndpoint) : getEndpoint()
    if (clientEndpoint && !endpoint) return { error: 'INVALID_API_ENDPOINT' }
    return {
      key: decodeKey(getHeader(req, 'x-ai-api-key', 4000)) || getKey(),
      endpoint: endpoint || getEndpoint(),
      model: getHeader(req, 'x-ai-model', 200) || getModel(),
    }
  }
  ...
  app.use(async (req, res, next) => {
    try {
      ...
      if (url.pathname === '/api/config' && req.method === 'GET') {
        ...
        if (providerConfig.error) { sendJson(res, 400, { error: providerConfig.error }); return }
        sendJson(res, 200, { hasKey: !!providerConfig.key, model: providerConfig.model, streaming: true, agent: true, materials: await listMaterials(), curriculum: await listDocuments() })
        return
      }
      ...
      if (url.pathname === '/api/summarize' && req.method === 'POST') {
        const providerConfig = getProviderConfig(req)
        if (providerConfig.error) { sendJson(res, 400, { error: providerConfig.error }); return }
        if (!providerConfig.key) { sendJson(res, 501, { code: 'NO_KEY' }); return }
