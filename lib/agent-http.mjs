...
function buildSystemPrompt(memory = '', branch = '') {
  return [
    ...
    'إذا كانت نية الطالبة اختباراً: اقرأ الصفحات المطلوبة أولًا ثم استدعِ present_quiz فورًا في نفس الرد — لا تكتب وصفًا أولًا ثم تقول "سأعرض الاختبار" ثم تتوقف. النموذج يجب أن يستدعي الأداة في الاستدعاء نفسه بعد القراءة مباشرة. لا تسأل أسئلة عامة — اسأل فقط عما ورد فعلاً في الصفحات.',
