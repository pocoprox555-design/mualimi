...
async function postOnce({ endpoint, key, body, stream, signal }) {
...
export async function* streamCompletion({ endpoint, key, model, messages, maxTokens, signal, onFirstToken }) {
  let lastErr = null;
  for (let attempt = 1; attempt <= RETRIES; attempt += 1) {
    if (signal?.aborted) throw signal.reason;
    try {
      const res = await postOnce({ endpoint, key, body: { model, messages, max_tokens: maxTokens, temperature: 0.6 }, stream: true, signal });
